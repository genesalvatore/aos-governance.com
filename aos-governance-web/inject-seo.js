...
function injectGEOContent(htmlPath, route) {
    ...
    const geoSummaries = {
        '/': `
            ...
            <p>AOS Governance is an open standard that provides a deterministic verification layer between AI agent intent and execution. It governs autonomous agents using code-based constitutional checks, not prompts. The standard implements a verifiable safety pipeline: every agent action is evaluated before execution, verified against a secure and verifiable policy framework by a deterministic script, and either permitted with an immutable audit log entry or blocked with a specific denial reason.</p>
