...
const POLICY_RESPONSES: PolicyEntry[] = [
    {
        slug: 'anthropic-mythos-preview-april-2026',
        date: 'April 7, 2026',
        respondingTo: 'Assessing Claude Mythos Preview\'s cybersecurity capabilities',
        respondingOrg: 'Anthropic',
        title: 'AOS Policy Response — Claude Mythos Preview & Project Glasswing',
        summary: 'Same-day architectural response to Anthropic\'s announcement of Claude Mythos Preview, a model capable of autonomously discovering and exploiting zero-day vulnerabilities in every major OS and browser. Maps Anthropic\'s findings to the AOS governance architecture, Deterministic Policy Gate, and patent portfolio.',
        tags: ['Zero-Day Exploitation', 'Syscall Baselining', 'Autonomous Offense', 'DPG'],
        patentsCited: 8,
        badge: 'Urgent',
    },
    {
        ...
        <>
            ...
            <section className="pb-24 bg-white md:-mt-10 relative z-10 rounded-t-[3rem] pt-12 md:pt-16">
                <div className="max-w-6xl mx-auto px-6">
                    {/* ─── THREE-CARD GRID: Standard + Responses ─── */}
                    <div className="grid lg:grid-cols-3 gap-8 items-stretch">
                        ...
                        <Link to="/policy/aos-standard" className="block group h-full">
                            <div className="border border-gray-200 rounded-[2rem] p-8 md:p-10 bg-[#f5f2eb] hover:bg-[#eae6d9] hover:border-transparent transition-colors shadow-sm hover:shadow-xl relative overflow-hidden flex flex-col h-full">
                                ...
                                <div className="flex-1 flex flex-col items-start gap-4 relative z-10 w-full mb-8">
                                    ...
                                    <h2 className="font-serif text-3xl md:text-4xl text-black leading-[1.05] mt-4">
                                        AOS Standard 1.0
                                    </h2>
                                    <p className="text-gray-700 leading-relaxed mt-2 text-base">
                                        The definitive architectural specification for deterministic AI governance. Five-layer stack for safe enterprise, physical, and orbital deployment.
                                    </p>
                                </div>
                                ...
                                <div className="mt-auto flex justify-between items-end relative z-10 w-full pt-6 border-t border-black/10">
                                    ...
                                    <span className="inline-flex items-center justify-center w-10 h-10 rounded-full bg-white border border-gray-300 text-black group-hover:scale-110 group-hover:bg-black group-hover:text-white transition-all shadow-sm shrink-0">
                                        →
                                    </span>
                                </div>
                            </div>
                        ...
                        {/* ─── CARD 2 & 3: Policy Response Cards ─── */}
                        {POLICY_RESPONSES.map((entry, idx) => (
                            <Link key={entry.slug} to={`/policy-response/${entry.slug}`} className="block group h-full">
                                <article className="border border-gray-200 rounded-[2rem] p-8 md:p-10 bg-white hover:border-black/30 transition-all duration-300 shadow-sm hover:shadow-xl relative flex flex-col h-full">
                                    ...
                                    <div className="flex-1 flex flex-col items-start relative z-10 w-full mb-8">
                                        ...
                                        <div className="flex items-center gap-3 mb-5">
                                            {idx === 0 ? (
                                                <div className="inline-block px-3 py-1 text-[10px] font-mono border border-red-200 rounded-full uppercase tracking-wider bg-red-50 text-red-700 shrink-0 flex items-center gap-2">
                                                    <div className="relative flex items-center justify-center w-2 h-2 shrink-0">
                                                        <span className="absolute inline-flex w-full h-full bg-red-400 rounded-full opacity-75 animate-ping"></span>
                                                        <span className="relative inline-flex w-1.5 h-1.5 bg-red-600 rounded-full"></span>
                                                    </div>
                                                    LATEST RESPONSE
                                                </div>
                                            ) : (
                                                <div className="inline-block px-3 py-1 text-[10px] font-mono border border-blue-200 rounded-full uppercase tracking-wider bg-blue-50 text-blue-700 shrink-0">
                                                    POLICY RESPONSE
                                                </div>
                                            )}
                                            <span className="text-[10px] text-gray-400 font-mono tracking-wider">{entry.date}</span>
                                        </div>
                                        ...
                                        <div className="text-sm text-gray-500 mb-4 bg-gray-50 px-3 py-2 rounded-lg border border-black/5 w-full">
                                            <span className="text-gray-400">Re:</span>{' '}
                                            <span className="italic font-medium text-black">{entry.respondingTo}</span>
                                            <span className="text-gray-400"> — {entry.respondingOrg}</span>
                                        </div>
                                    ...
                                    <div className="mt-auto flex flex-wrap sm:flex-nowrap items-center justify-between gap-4 pt-5 border-t border-gray-100 w-full relative z-10">
                    ...
                    {POLICY_RESPONSES.length > 2 && (
                        <div className="mt-16 md:mt-24 max-w-4xl mx-auto">
                            <section className="space-y-6">
                                <div className="border-b border-gray-200 pb-4 mb-8">
                                    <h3 className="text-sm font-mono uppercase tracking-widest text-gray-500">Response Archive ({POLICY_RESPONSES.length - 2})</h3>
                                </div>
                                <div className="space-y-6">
                                    {POLICY_RESPONSES.slice(2).map((entry) => (
